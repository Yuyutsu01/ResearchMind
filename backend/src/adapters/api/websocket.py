...
async def websocket_research(websocket: WebSocket, session_id: int):
    ...
    try:
        ...
        # Keep connection open for interactive reading annotations
        print(f"[WebSocket] Session #{session_id} entering interactive reading mode.")
        while True:
            ...
            if msg.get("type") == "page_visible":
                ...
                # Dynamic swarm explanation execution
                from src.domain.agents.explain_selection import explain_selection
                explanation = explain_selection(blackboard, sel_text, sel_type, obj_metadata)
                
                # Stream the progressive annotation response back
                await send_client_payload({
